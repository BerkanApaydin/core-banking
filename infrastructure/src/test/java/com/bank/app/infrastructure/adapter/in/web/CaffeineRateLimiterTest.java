...
class CaffeineRateLimiterTest {
    ...
    void shouldNotAddRejectedRequestsToWindow() {
        ...
        assertTrue(limiter.tryAcquire("client")); // at t=1000
        testClock.advance(100);
        assertTrue(limiter.tryAcquire("client")); // at t=1100
        testClock.advance(100);
        assertFalse(limiter.tryAcquire("client")); // at t=1200 (rejected, should not be added)

        testClock.advance(350); // t = 1550 (request at t=1000 is expired, request at t=1100 is active)
        assertTrue(limiter.tryAcquire("client")); // should be allowed because size is 1 (only t=1100 remains)
    }
