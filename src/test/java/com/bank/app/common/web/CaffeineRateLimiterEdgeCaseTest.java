...
class CaffeineRateLimiterEdgeCaseTest {
    ...
    void shouldAllowRequestWhenUnderLimit() {
        CaffeineRateLimiter limiter = new CaffeineRateLimiter(5, 60000, Clock.systemDefaultZone());
        assertTrue(limiter.tryAcquire("client-1"));
    }

    @Test
    void shouldBlockRequestWhenOverLimit() {
        CaffeineRateLimiter limiter = new CaffeineRateLimiter(2, 60000, Clock.systemDefaultZone());
        assertTrue(limiter.tryAcquire("client-1"));
        assertTrue(limiter.tryAcquire("client-1"));
    ...
    void shouldAllowDifferentClientsIndependently() {
        CaffeineRateLimiter limiter = new CaffeineRateLimiter(1, 60000, Clock.systemDefaultZone());
        assertTrue(limiter.tryAcquire("client-1"));
    ...
    void shouldResetAfterWindowExpires() {
        ...
        CaffeineRateLimiter limiter = new CaffeineRateLimiter(1, 50000, clock);

        assertTrue(limiter.tryAcquire("client-1"));
        assertFalse(limiter.tryAcquire("client-1"));

        Clock laterClock = Clock.fixed(now.plusMillis(50001), ZoneId.systemDefault());
        CaffeineRateLimiter limiterWithLaterClock = new CaffeineRateLimiter(1, 50000, laterClock);
    ...
    void shouldHandleHighVolumeClients() {
        CaffeineRateLimiter limiter = new CaffeineRateLimiter(1000, 60000, Clock.systemDefaultZone());
