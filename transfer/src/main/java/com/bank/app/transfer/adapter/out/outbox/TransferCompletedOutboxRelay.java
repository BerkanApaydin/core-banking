...
@Component
public class TransferCompletedOutboxRelay implements OutboxEventPort {

    private static final Logger log = LoggerFactory.getLogger(TransferCompletedOutboxRelay.class);
    ...
    public void handle(EventEntry event) {
        ...
        if (!idempotencyPort.tryCreate(dedupKey, LocalDateTime.now())) {
            log.info("Duplicate outbox event detected, skipping. handler=TransferCompletedOutboxRelay, eventId={}", event.id());
            return;
        }
        ...
        try {
            ...
            throw new RuntimeException("TransferCompletedOutboxRelay failed", e);
        }
    }
}
