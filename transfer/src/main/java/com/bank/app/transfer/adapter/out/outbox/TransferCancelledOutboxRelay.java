...
@Component
public class TransferCancelledOutboxRelay implements OutboxEventPort {

    private static final Logger log = LoggerFactory.getLogger(TransferCancelledOutboxRelay.class);
    ...
    public void handle(EventEntry event) {
        ...
        if (!idempotencyPort.tryCreate(dedupKey, LocalDateTime.now())) {
            log.info("Duplicate outbox event detected, skipping. handler=TransferCancelledOutboxRelay, eventId={}", event.id());
            return;
        }
        ...
        try {
            ...
            throw new RuntimeException("TransferCancelledOutboxRelay failed", e);
        }
    }
}
